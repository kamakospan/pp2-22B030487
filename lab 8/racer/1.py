...
f2 = pg.font.Font('Gemstone.ttf', 36)

screen=pg.display.set_mode((SCR_W, SCR_H))
pg.display.set_caption("racer")

mix.music.load("Subway Surfers OST Extended (256  kbps).mp3")
    ...
    def move(self):
        ...
        if self.rect.left > 0:
              if pressed_keys[pg.K_LEFT]:
                  self.rect.move_ip(-1*(JAKESPEED+SCORECOINS//10), 0)
        if self.rect.right < SCR_W:        
              if pressed_keys[pg.K_RIGHT]:
                  self.rect.move_ip(JAKESPEED+SCORECOINS//10, 0)
    def draw(self, surface):
        if JAKEMODE:
...
while True:
    ...
    if deadscreen:
        ...
        rect=tex1.get_rect()
        rect.center=(200,480)
        tex2=f2.render("COIN:{}".format(str(SCORECOINS)),True,(255, 255, 255))
        rect2=tex2.get_rect()
